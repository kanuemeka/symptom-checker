...
public class AssessmentController {
    ...
    @PostMapping("/start")
    public ResponseEntity<Assessment>startAssessment(@Valid @RequestBody NewAssessment newAssessment) {
        Assessment assessment = assessmentStarter.beginAssessment(newAssessment.getUserId(), newAssessment.getInitialSymptoms());
    ...
    public ResponseEntity<String> answerQuestion(@PathVariable(name = "assessmentId") String assessmentId, @Valid @RequestBody Answer answer){
        String nextQuestionId = responseRecorder.recordResponse(
                assessmentId, answer.getQuestionId(), answer.getAnswerOption().toString());
